// functions/.netlify/functions/submit-project.js
...
export async function onRequest(context) {
  ...
  const SIGN_SECRET = env.CERT_SIGNING_SECRET;
  const PREFIX = env.VOLYNX_CERT_PREFIX || "VLX-DJ";
  ...
  // compra precisa estar marcada como ativa pelo webhook (Hotmart)
  const purchaseKey = `purchase:${email}`;
  const purchase = await store.get(purchaseKey, { type: "json" });

  if (!purchase?.isActive) {
    return unauthorized("Compra não encontrada ou não elegível (precisa estar APROVADA/COMPLETA).");
  }
  ...
  if (!repoInfo.ok) {
    return badRequest("Não consegui acessar esse repo (confira se é público e o link está correto).", repoInfo);
  }
  ...
  const runs = await githubRequest(
    `/repos/${owner}/${repo}/actions/workflows/volynx-validate.yml/runs?per_page=1`,
    GITHUB_TOKEN
  );
  if (!runs.ok) {
    return badRequest("Não consegui ler os runs do GitHub Actions. O Actions está habilitado?", runs);
  }

  const last = runs.json?.workflow_runs?.[0];
  if (!last) {
    return badRequest("Ainda não há execução do workflow. Faça um push para disparar a validação.");
  }
  ...
  const fingerprint = await hmacSha256Hex(SIGN_SECRET, JSON.stringify(payload));
